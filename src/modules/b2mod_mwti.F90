...
#   include <netcdf.inc>
    ...
    integer imap(maxvdims), iret, iatm, imol
    ...
    real (kind=R8), allocatable :: &
         ...
         dab3dl(:,:), dmb3dl(:,:), tab3dl(:,:), tmb3dl(:,:), &
         dab3di(:,:), dmb3di(:,:), tab3di(:,:), tmb3di(:,:), &
         dab3da(:,:), dmb3da(:,:), tab3da(:,:), tmb3da(:,:), &
         dab3dr(:,:), dmb3dr(:,:), tab3dr(:,:), tmb3dr(:,:), &
         dab3dtl(:,:), dmb3dtl(:,:), tab3dtl(:,:), tmb3dtl(:,:), &
         dab3dtr(:,:), dmb3dtr(:,:), tab3dtr(:,:), tmb3dtr(:,:), &
         dn3di(:,:), dn3da(:,:), dp3di(:,:), dp3da(:,:), &
         vs3di(:,:), vs3da(:,:), vx3di(:,:), vx3da(:,:), vy3di(:,:), vy3da(:,:)
    real (kind=R8) :: &
    ...
    if (ncall.eq.0) then
      ...
      if (.not. allocated(dsl)) then
        allocate(dsl(-1:ny), dsi(-1:ny), dsa(-1:ny), dsr(-1:ny), dstl(-1:ny), dstr(-1:ny), &
                 dsLT(-1:ny), dsRT(-1:ny), dsTLT(-1:ny), dsTRT(-1:ny), &
                 dsLP(-1:ny), dsRP(-1:ny), dsTLP(-1:ny), dsTRP(-1:ny))
      end if
      call output_ds(ny, -1,+target_offset,jsep,iylstrt,iylend,'dsl',dsl)
      call output_ds(ny,jxi,0,jsep,iyistrt,iyiend,'dsi',dsi)
      call output_ds(ny,jxa,0,jsep,iyastrt,iyaend,'dsa',dsa)
      call output_ds(ny, nx,-target_offset,jsep,iyrstrt,iyrend,'dsr',dsr)
      if (nnreg(0).ge.7) then
        ...
        do while (leftix(ixtr,max(topcut(1),topcut(2))).ne.-2 .and. ixtr.lt.nx)
          ixtr=ixtr+1
        enddo
        call output_ds(ny,ixtl,-target_offset,jsep,iytlstrt,iytlend,'dstl',dstl)
        call output_ds(ny,ixtr,+target_offset,jsep,iytrstrt,iytrend,'dstr',dstr)
#ifndef NO_CDF
      ...
      do iy=iylstrt,iylend
        if (region(-1,iy,0).ne.0) then
          write(99,*) gs(rightix(-1,iy),rightiy(-1,iy),0)*qc(rightix(-1,iy),rightiy(-1,iy))
          dsLP(iy) = gs(rightix(-1,iy),rightiy(-1,iy),0)*qc(rightix(-1,iy),rightiy(-1,iy))
        else if (region(-1+target_offset,iy,0).ne.0) then
          write(99,*) &
        &  gs(rightix(-1+target_offset,iy),rightiy(-1+target_offset,iy),0)* &
        &  qc(rightix(-1+target_offset,iy),rightiy(-1+target_offset,iy))
          dsLP(iy) = &
        &  gs(rightix(-1+target_offset,iy),rightiy(-1+target_offset,iy),0)* &
        &  qc(rightix(-1+target_offset,iy),rightiy(-1+target_offset,iy))
        endif
      ...
      do iy=iyrstrt,iyrend
        if (region(nx,iy,0).ne.0) then
          ...
          dsRP(iy) = gs(nx,iy,0)*qc(nx,iy)
        else if (region(nx-target_offset,iy,0).ne.0) then
          write(99,*) gs(nx-target_offset,iy,0)*qc(nx-target_offset,iy)
          dsRP(iy) = gs(nx-target_offset,iy,0)*qc(nx-target_offset,iy)
        endif
      enddo
      close(99)
      if (nnreg(0).ge.7) then
        ...
        do iy=iytlstrt,iytlend
          ...
          dsTLP(iy) = gs(ixtl,iy,0)*qc(ixtl,iy)
        enddo
        close(99)
        open(99,file='dsTRP')
        dsTRP = 0.0_R8
        do iy=iytrstrt,iytrend
          ...
          dsTRP(iy) = gs(rightix(ixtr,iy),rightiy(ixtr,iy),0)*qc(rightix(ixtr,iy),rightiy(ixtr,iy))
        enddo
        close(99)
      endif
#ifndef NO_CDF
      if (b2time.gt.0) then
        ...
        if (.not.ex.or.stim.ge.0.0_R8) then
          ...
          call b2crtimecdf(filename, &
            nx, ny, nybl, nytl, nytr, nybr, nya, nyi, nc, ns, nnatmi, nnmoli, write_2d, &
            ncid, .false., iret)
          ...
          call b2crtimecdf(filename, &
            nx, ny, nybl, nytl, nytr, nybr, nya, nyi, nc, ns, nnatmi, nnmoli, &
    ...
    namxip = 0.0_R8; nemxip = 0.0_R8; temxip = 0.0_R8; timxip = 0.0_R8; pomxip = 0.0_R8; pwmxip = 0.0_R8; tpmxip = 0.0_R8
    ix = -1 ! 1
    ...
    do iy = iylstrt,iylend
      ...
      do is = 0, ns-1
         namxip(is+1,1) = max(namxip(is+1,1), na(ix_off,iy,is))
      end do
    ...
    namxap = 0.0_R8; nemxap = 0.0_R8; temxap = 0.0_R8; timxap = 0.0_R8; pomxap = 0.0_R8; pwmxap = 0.0_R8; tpmxap = 0.0_R8
    ix = nx ! 2
    ix_off  = ix - target_offset
    do iy = iyrstrt,iyrend
      ...
      do is = 0, ns-1
         namxap(is+1,1) = max(namxap(is+1,1), na(ix_off,iy,is))
      end do
    ...
    if(nncut.ge.2) then
      ...
      do iy = iytrstrt,iytrend
        ...
        do is = 0, ns-1
           namxap(is+1,2) = max(namxap(is+1,2), na(ix_off,iy,is))
        end do
      ...
      do iy = iytlstrt,iytlend
        ...
        do is = 0, ns-1
           namxip(is+1,2) = max(namxip(is+1,2), na(ix_off,iy,is))
        end do
    ...
    allocate(na3dl(nybl, ns))
    allocate(na3di(nyi, ns))
    allocate(na3da(nya, ns))
    allocate(na3dr(nybr, ns))
    allocate(dn3di(nyi, ns))
    allocate(dn3da(nya, ns))
    allocate(dp3di(nyi, ns))
    allocate(dp3da(nya, ns))
    allocate(vx3di(nyi, ns))
    allocate(vx3da(nya, ns))
    allocate(vy3di(nyi, ns))
    allocate(vy3da(nya, ns))
    allocate(vs3di(nyi, ns))
    allocate(vs3da(nya, ns))
    allocate(fn3dl(nybl, ns))
    allocate(fn3dr(nybr, ns))
    ...
    do is = 0, ns-1
      na3dl(:, is+1) = na(-1+target_offset,iylstrt:iylend,is)
      na3di(:, is+1) = na(jxi,iyistrt:iyiend,is)
      na3da(:, is+1) = na(jxa,iyastrt:iyaend,is)
      na3dr(:, is+1) = na(nx-target_offset,iyrstrt:iyrend,is)
      dn3di(:, is+1) = dna0(jxi,iyistrt:iyiend,is)
      dn3da(:, is+1) = dna0(jxa,iyastrt:iyaend,is)
      dp3di(:, is+1) = dpa0(jxi,iyistrt:iyiend,is)*(rza(jxi,iyistrt:iyiend,is)*te(jxi,iyistrt:iyiend)+ti(jxi,iyistrt:iyiend))
      dp3da(:, is+1) = dpa0(jxa,iyastrt:iyaend,is)*(rza(jxa,iyastrt:iyaend,is)*te(jxa,iyastrt:iyaend)+ti(jxa,iyastrt:iyaend))
      vx3di(:, is+1) = vla0(jxi,iyistrt:iyiend,0,is)
      vx3da(:, is+1) = vla0(jxa,iyastrt:iyaend,0,is)
      vy3di(:, is+1) = vla0(jxi,iyistrt:iyiend,1,is)
      vy3da(:, is+1) = vla0(jxa,iyastrt:iyaend,1,is)
      vs3di(:, is+1) = vsa0(jxi,iyistrt:iyiend,is)/(mp*am(is)*na(jxi,iyistrt:iyiend,is))
      vs3da(:, is+1) = vsa0(jxa,iyastrt:iyaend,is)/(mp*am(is)*na(jxa,iyastrt:iyaend,is))
      fn3dl(:, is+1) = fna(0,iylstrt:iylend,0,is)
      fn3dr(:, is+1) = fna(nx,iyrstrt:iyrend,0,is)
    end do

    if (nnreg(0).ge.7) then

      allocate(na3dtl(nytl, ns))
      allocate(na3dtr(nytr, ns))
      allocate(fn3dtl(nytl, ns))
      allocate(fn3dtr(nytr, ns))
      ...
      do is = 0, ns-1
        na3dtl(:, is+1) = na(ixtl-target_offset,iytlstrt:iytlend,is)
        na3dtr(:, is+1) = na(ixtr+target_offset,iytrstrt:iytrend,is)
        fn3dtl(:, is+1) = fna(ixtl,iytlstrt:iytlend,0,is)
        fn3dtr(:, is+1) = fna(ixtr+1,iytrstrt:iytrend,0,is)
      end do
...
!wdk only write time data if lwti is true
    if (lwti) then
      ...
      call rwcdf(rw,ncid,'ntstep',imap,tstepn,iret)
      jxin(1) = jxi
      call rwcdf(rw,ncid,'jxi',imap,jxin,iret)
      jxan(1) = jxa
      ...
      if (write_2d .ge. 1) then
        ...
        imap(4)=1
        imap(5)=1
        call rwcdf(rw,ncid,'na2d',imap,na,iret)
        call rwcdf(rw,ncid,'ne2d',imap,ne,iret)
        call rwcdf(rw,ncid,'te2d',imap,te,iret)
        call rwcdf(rw,ncid,'ti2d',imap,ti,iret)
        if (write_2d .ge. 2) then
          call rwcdf(rw,ncid,'po2d',imap,po,iret)
          call rwcdf(rw,ncid,'kin2d',imap,kinrgy,iret)
          call rwcdf(rw,ncid,'rsahi2d',imap,rsahi,iret)
          call rwcdf(rw,ncid,'rsana2d',imap,rsana,iret)
          call rwcdf(rw,ncid,'rrahi2d',imap,rrahi,iret)
          call rwcdf(rw,ncid,'rrana2d',imap,rrana,iret)
          call rwcdf(rw,ncid,'rcxhi2d',imap,rcxhi,iret)
          call rwcdf(rw,ncid,'rcxna2d',imap,rcxna,iret)
          call rwcdf(rw,ncid,'rqrad2d',imap,rqrad,iret)
          call rwcdf(rw,ncid,'fhe2d',imap,fhe,iret)
          call rwcdf(rw,ncid,'fhi2d',imap,fhi,iret)
          call rwcdf(rw,ncid,'fch2d',imap,fch,iret)
          call rwcdf(rw,ncid,'fna2d',imap,fna,iret)
        endif
      endif
      imap(1)=1
      imap(2)=1
      ...
      call rwcdf(rw,ncid,'nasepm',imap,nasepm,iret)
      ...
      if (nnatmi.gt.0) then
        call rwcdf(rw,ncid,'dabsepm',imap,dabsepm,iret)
        call rwcdf(rw,ncid,'tabsepm',imap,tabsepm,iret)
      endif
      if (nnmoli.gt.0) then
        call rwcdf(rw,ncid,'dmbsepm',imap,dmbsepm,iret)
        call rwcdf(rw,ncid,'tmbsepm',imap,tmbsepm,iret)
      endif
      ...
      call rwcdf(rw,ncid,'nasepa',imap,nasepa,iret)
      ...
      if (nnatmi.gt.0) then
        call rwcdf(rw,ncid,'dabsepa',imap,dabsepa,iret)
        call rwcdf(rw,ncid,'tabsepa',imap,tabsepa,iret)
      endif
      if (nnmoli.gt.0) then
        call rwcdf(rw,ncid,'dmbsepa',imap,dmbsepa,iret)
        call rwcdf(rw,ncid,'tmbsepa',imap,tmbsepa,iret)
      endif
      ...
      call rwcdf(rw,ncid,'namxap',imap,namxap,iret)
    ...
    !
      ...
      imap(2)=1
      imap(3)=1
      call rwcdf(rw,ncid,'na3dl',imap,na3dl,iret)
      call rwcdf(rw,ncid,'fn3dl',imap,fn3dl,iret)
      imap(1)=nx+2     ! i
      imap(2)=1
      ...
      imap(2)=1
      imap(3)=1
      call rwcdf(rw,ncid,'na3di',imap,na3di,iret)
      imap(1)=nx+2     ! a
      imap(2)=1
      ...
      imap(2)=1
      imap(3)=1
      call rwcdf(rw,ncid,'na3da',imap,na3da,iret)
      imap(1)=nx+2     ! br
      imap(2)=1
      ...
      imap(2)=1
      imap(3)=1
      call rwcdf(rw,ncid,'na3dr',imap,na3dr,iret)
      call rwcdf(rw,ncid,'fn3dr',imap,fn3dr,iret)
      if (nnreg(0).ge.7) then
        imap(1)=nx+2     ! tr
        imap(2)=1
        ...
        imap(2)=1
        imap(3)=1
        call rwcdf(rw,ncid,'na3dtr',imap,na3dtr,iret)
        call rwcdf(rw,ncid,'fn3dtr',imap,fn3dtr,iret)
        imap(1)=nx+2     ! tl
        imap(2)=1
        ...
        imap(2)=1
        imap(3)=1
        call rwcdf(rw,ncid,'na3dtl',imap,na3dtl,iret)
        call rwcdf(rw,ncid,'fn3dtl',imap,fn3dtl,iret)
      endif
    !
      imap(1)=1
      imap(2)=1
    ...
    !
      ...
      if (nnatmi.gt.0) then
        allocate(dab3dl(nybl, nnatmi))
        allocate(dab3di(nyi, nnatmi))
        allocate(dab3da(nya, nnatmi))
        allocate(dab3dr(nybr, nnatmi))
        allocate(tab3dl(nybl, nnatmi))
        allocate(tab3di(nyi, nnatmi))
        allocate(tab3da(nya, nnatmi))
        allocate(tab3dr(nybr, nnatmi))
        if (nnreg(0).ge.7) then
          allocate(dab3dtl(nytl, nnatmi))
          allocate(dab3dtr(nytr, nnatmi))
          allocate(tab3dtl(nytl, nnatmi))
          allocate(tab3dtr(nytr, nnatmi))
        endif
      endif

      if (nnmoli.gt.0) then
        allocate(dmb3dl(nybl, nnmoli))
        allocate(dmb3di(nyi, nnmoli))
        allocate(dmb3da(nya, nnmoli))
        allocate(dmb3dr(nybr, nnmoli))
        allocate(tmb3dl(nybl, nnmoli))
        allocate(tmb3di(nyi, nnmoli))
        allocate(tmb3da(nya, nnmoli))
        allocate(tmb3dr(nybr, nnmoli))
        if (nnreg(0).ge.7) then
          allocate(dmb3dtl(nytl, nnmoli))
          allocate(dmb3dtr(nytr, nnmoli))
          allocate(tmb3dtl(nytl, nnmoli))
          allocate(tmb3dtr(nytr, nnmoli))
        endif
      endif
      ...
      if (nnatmi.gt.0) then
        ...
        imap(2)=1
        imap(3)=1
        call rwcdf(rw,ncid,'dab3dl',imap,dab3dl,iret)
        call rwcdf(rw,ncid,'dab3di',imap,dab3di,iret)
        call rwcdf(rw,ncid,'dab3da',imap,dab3da,iret)
        call rwcdf(rw,ncid,'dab3dr',imap,dab3dr,iret)
        call rwcdf(rw,ncid,'tab3dl',imap,tab3dl,iret)
        call rwcdf(rw,ncid,'tab3di',imap,tab3di,iret)
        call rwcdf(rw,ncid,'tab3da',imap,tab3da,iret)
        call rwcdf(rw,ncid,'tab3dr',imap,tab3dr,iret)
        if (nnreg(0).ge.7) then
          call rwcdf(rw,ncid,'dab3dtl',imap,dab3dtl,iret)
          call rwcdf(rw,ncid,'dab3dtr',imap,dab3dtr,iret)
          call rwcdf(rw,ncid,'tab3dtl',imap,tab3dtl,iret)
          call rwcdf(rw,ncid,'tab3dtr',imap,tab3dtr,iret)
        endif
      endif
      ...
      if (nnmoli.gt.0) then
        ...
        imap(2)=1
        imap(3)=1
        call rwcdf(rw,ncid,'dmb3dl',imap,dmb3dl,iret)
        call rwcdf(rw,ncid,'dmb3di',imap,dmb3di,iret)
        call rwcdf(rw,ncid,'dmb3da',imap,dmb3da,iret)
        call rwcdf(rw,ncid,'dmb3dr',imap,dmb3dr,iret)
        call rwcdf(rw,ncid,'tmb3dl',imap,tmb3dl,iret)
        call rwcdf(rw,ncid,'tmb3di',imap,tmb3di,iret)
        call rwcdf(rw,ncid,'tmb3da',imap,tmb3da,iret)
        call rwcdf(rw,ncid,'tmb3dr',imap,tmb3dr,iret)
        if (nnreg(0).ge.7) then
          call rwcdf(rw,ncid,'dmb3dtl',imap,dmb3dtl,iret)
          call rwcdf(rw,ncid,'dmb3dtr',imap,dmb3dtr,iret)
          call rwcdf(rw,ncid,'tmb3dtl',imap,tmb3dtl,iret)
          call rwcdf(rw,ncid,'tmb3dtr',imap,tmb3dtr,iret)
        endif
      endif
      ...
      imap(2)=1
      imap(3)=1
      call rwcdf(rw,ncid,'dn3di',imap,dn3di,iret)
      call rwcdf(rw,ncid,'dn3da',imap,dn3da,iret)
      call rwcdf(rw,ncid,'dp3di',imap,dp3di,iret)
      call rwcdf(rw,ncid,'dp3da',imap,dp3da,iret)
      ...
      call rwcdf(rw,ncid,'vx3di',imap,vx3di,iret)
      call rwcdf(rw,ncid,'vx3da',imap,vx3da,iret)
      call rwcdf(rw,ncid,'vy3di',imap,vy3di,iret)
      call rwcdf(rw,ncid,'vy3da',imap,vy3da,iret)
      call rwcdf(rw,ncid,'vs3di',imap,vs3di,iret)
      call rwcdf(rw,ncid,'vs3da',imap,vs3da,iret)
...
!wdk averages
      ...
      imap(2)=1
      imap(3)=nncutmax
      call rwcdf(rw,ncid,'nasepm_av',imap,nasepm_av,iret)
      call rwcdf(rw,ncid,'nasepi_av',imap,nasepi_av,iret)
      call rwcdf(rw,ncid,'nasepa_av',imap,nasepa_av,iret)
      call rwcdf(rw,ncid,'namxip_av',imap,namxip_av,iret)
      call rwcdf(rw,ncid,'namxap_av',imap,namxap_av,iret)
      if (nnatmi.gt.0) then
        call rwcdf(rw,ncid,'dabsepm_av',imap,dabsepm_av,iret)
        call rwcdf(rw,ncid,'tabsepm_av',imap,tabsepm_av,iret)
        call rwcdf(rw,ncid,'dabsepi_av',imap,dabsepi_av,iret)
        call rwcdf(rw,ncid,'tabsepi_av',imap,tabsepi_av,iret)
        call rwcdf(rw,ncid,'dabsepa_av',imap,dabsepa_av,iret)
        call rwcdf(rw,ncid,'tabsepa_av',imap,tabsepa_av,iret)
      endif
      if (nnmoli.gt.0) then
        call rwcdf(rw,ncid,'dmbsepm_av',imap,dmbsepm_av,iret)
        call rwcdf(rw,ncid,'tmbsepm_av',imap,tmbsepm_av,iret)
        call rwcdf(rw,ncid,'dmbsepi_av',imap,dmbsepi_av,iret)
        call rwcdf(rw,ncid,'tmbsepi_av',imap,tmbsepi_av,iret)
        call rwcdf(rw,ncid,'dmbsepa_av',imap,dmbsepa_av,iret)
        call rwcdf(rw,ncid,'tmbsepa_av',imap,tmbsepa_av,iret)
      endif
...
!wdk standard deviations
      ...
      imap(2)=1
      imap(3)=nncutmax
      call rwcdf(rw,ncid,'nasepm_std',imap,nasepm_std,iret)
      call rwcdf(rw,ncid,'nasepi_std',imap,nasepi_std,iret)
      call rwcdf(rw,ncid,'nasepa_std',imap,nasepa_std,iret)
      call rwcdf(rw,ncid,'namxip_std',imap,namxip_std,iret)
      call rwcdf(rw,ncid,'namxap_std',imap,namxap_std,iret)
      if (nnatmi.gt.0) then
        call rwcdf(rw,ncid,'dabsepm_std',imap,dabsepm_std,iret)
        call rwcdf(rw,ncid,'tabsepm_std',imap,tabsepm_std,iret)
        call rwcdf(rw,ncid,'dabsepi_std',imap,dabsepi_std,iret)
        call rwcdf(rw,ncid,'tabsepi_std',imap,tabsepi_std,iret)
        call rwcdf(rw,ncid,'dabsepa_std',imap,dabsepa_std,iret)
        call rwcdf(rw,ncid,'tabsepa_std',imap,tabsepa_std,iret)
      endif
      if (nnmoli.gt.0) then
        call rwcdf(rw,ncid,'dmbsepm_std',imap,dmbsepm_std,iret)
        call rwcdf(rw,ncid,'tmbsepm_std',imap,tmbsepm_std,iret)
        call rwcdf(rw,ncid,'dmbsepi_std',imap,dmbsepi_std,iret)
        call rwcdf(rw,ncid,'tmbsepi_std',imap,tmbsepi_std,iret)
        call rwcdf(rw,ncid,'dmbsepa_std',imap,dmbsepa_std,iret)
        call rwcdf(rw,ncid,'tmbsepa_std',imap,tmbsepa_std,iret)
      endif
      iret = nf_close(ncid)
      call check_cdf_status(iret)
    endif !lwav
  ...
  subroutine b2crtimecdf(filename, &
   ...
   ncid, batch_only, iret)
    use b2mod_constants
#   include <netcdf.inc>
    integer nx, ny, nybl, nytl, nytr, nybr, nya, nyi, nc, ns, nnatmi, nnmoli, iret
    ...
    integer  ncid
    ! dimension ids
    integer :: nxdim, nydim, nsdim, natmdim, nmoldim, timedim, batchdim, &
    ...
    integer :: ntstepid, jxiid, jxaid, jsepid, timesaid, &
         ...
         feesappid, feisappid, na3dlid, ne3dlid, te3dlid, ti3dlid, &
         ...
         na3diid, ne3diid, te3diid, ti3diid, na3daid, ne3daid, te3daid, ti3daid, na3drid, ne3drid, te3drid, &
         ti3drid, an3dlid, mn3dlid, an3diid, mn3diid, an3daid, mn3daid, &
         an3drid, mn3drid, fn3dlid, fe3dlid, fi3dlid, fn3drid, fe3drid, &
         fi3drid, na3dtlid, ne3dtlid, te3dtlid, ti3dtlid, na3dtrid, ne3dtrid, te3dtrid, &
    ...
    if (.not.batch_only) then
      ...
      call check_cdf_status(iret)
      iret = nf_def_var(ncid, 'jxi', NCDOUBLE, 0, dims, jxiid)
      call check_cdf_status(iret)
      iret = nf_def_var(ncid, 'jxa', NCDOUBLE, 0, dims, jxaid)
      call check_cdf_status(iret)
      iret = nf_def_var(ncid, 'jsep', NCDOUBLE, 0, dims, jsepid)
      call check_cdf_status(iret)
      dims(1) = timedim
      ...
      if (write_2d .ge. 1) then
        ...
        dims(3) = nsdim
        dims(4) = timedim
        iret = nf_def_var(ncid, 'na2d', NCDOUBLE, 4, dims, na2did)
        call check_cdf_status(iret)
        ...
        dims(2) = nydim
        dims(3) = timedim
        iret = nf_def_var(ncid, 'ne2d', NCDOUBLE, 3, dims, ne2did)
        call check_cdf_status(iret)
        iret = nf_put_att_text(ncid, ne2did, 'long_name', 2, 'ne')
        call check_cdf_status(iret)
        iret = nf_put_att_text(ncid, ne2did, 'units', 4, 'm^-3')
        call check_cdf_status(iret)
        iret = nf_def_var(ncid, 'te2d', NCDOUBLE, 3, dims, te2did)
        call check_cdf_status(iret)
        ...
        call check_cdf_status(iret)
        iret = nf_def_var(ncid, 'ti2d', NCDOUBLE, 3, dims, ti2did)
        call check_cdf_status(iret)
        ...
        call check_cdf_status(iret)
        if (write_2d .ge. 2) then
          iret = nf_def_var(ncid, 'po2d', NCDOUBLE, 3, dims, po2did)
          call check_cdf_status(iret)
          ...
          dims(4) = nsdim
          dims(5) = timedim
          iret = nf_def_var(ncid, 'fna2d', NCDOUBLE, 5, dims, fna2did)
          call check_cdf_status(iret)
...
#endif
!
  subroutine output_ds(ny,iref,target_offset,jsep,iystart,iyend,filename,ds)
    use b2mod_geo
    use b2mod_indirect
    implicit none
    integer ny,iref,jsep,iystart,iyend,target_offset
    real (kind=R8), intent(out) :: ds(-1:ny)
